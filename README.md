...
- **`postcodes_master_wide.csv`** — Exploration-only wider postcode extract, generated from the same national ONSPD file by keeping postcodes within a configurable distance buffer of any lookup hospital in `hospitals_refined.csv`.
...
- **Hospital directory** — view every hospital in `hospitals_refined.csv`, including network, care level and specialties.
...
|------|--------|-------------|
| `postcodes_master.csv` | ONS Postcode Directory May 2026 (`ONSPD_MAY_2026_UK.csv`) | London-area postcodes (`rgn25cd = E12000007`) with latitude/longitude coordinates; generated by `extract_postcode.py` |
| `postcodes_master_wide.csv` | ONS Postcode Directory May 2026 (`ONSPD_MAY_2026_UK.csv`) | Wider exploration extract within a hospital-distance buffer; generated by `extract_postcode.py --mode hospital-buffer` |
...
1. **Generate postcode lookups (lookup profile for app/map/static)**
   ...
   For the wider-catchments exploration branch, first generate the wider postcode extract:
   ```
   python3 extract_postcode.py --mode hospital-buffer --output postcodes_master_wide.csv --buffer-km 50
   ```

   Then run the lookup against that wider file:
   ```
   python3 postcode_lookup.py --profile lookup --postcodes-csv postcodes_master_wide.csv
   ```
